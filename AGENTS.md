...
| `wv quality hotspots`      | Ranked hotspot report (ev, trend, ownership)                              |
| `wv quality diff`          | Delta vs previous scan with trend direction                               |
| `wv quality functions`     | Per-function CC report with dispatch tagging                              |
...
| `weave_recover`           | Resume incomplete multi-step operations                                |
...
| `weave_delete`            | Remove node permanently (requires force=true)                          |
| `weave_show`              | Single-node detail view (JSON output)                                  |
...
| `weave_guide`             | Workflow quick reference (topic: workflow\|github\|learnings\|context) |
...
| `weave_quality_scan`      | Codebase quality metrics scan                                          |
...
- **Metadata key in JSON:** Both `wv show --json` and `wv list --json` return metadata under
  `"metadata"`. Parse with: `jq '.[0].metadata | fromjson | .field'`.
...
- **⚠ Plan mode bypass:** When given a detailed spec or release plan, do NOT skip the Weave
  workflow. Create child nodes per phase (`wv add --parent=<epic> --gh`), claim each before coding
  (`wv work`), close with learnings (`wv done --learning`). Never use `--no-verify` or
