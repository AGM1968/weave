...
- **Architecture and IPC boundary reconciliation** — documentation now distinguishes live graph
  authority from Git-tracked durability, execution hosts from inspection providers, and the current
  Bash compatibility implementation from the target persistent typed IPC service.
- **Bounded Bash suite concurrency** — `tests/run-all.sh` runs in configurable worker batches
  (`WV_TEST_JOBS`, default 4), preventing resource contention from masking suite results.
...
- **Cross-agent stop-hook scoping** - the Claude stop hook now blocks only active work claimed by
  the current agent (or unclaimed work), so parallel sandboxed Claude, Codex, Copilot, and MCP
  sessions sharing a Weave hot zone no longer prevent each other from ending normally.
