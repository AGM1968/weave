...
describe("Weave MCP Server", () => {
  ...
  describe("tools/list", () => {
    it("should list all 23 tools (default scope=all)", async () => {
...
describe("Weave MCP Server --scope=graph", () => {
  ...
  it("should only expose graph tools", async () => {
    ...
    expect(toolNames).toEqual(
      expect.arrayContaining(["weave_add", "weave_link", "weave_done", "weave_batch_done", "weave_list", "weave_resolve", "weave_update"])
    );
...
describe("Weave MCP Server --scope=session", () => {
  ...
  it("should only expose session tools", async () => {
    ...
    expect(toolNames).toEqual(
      expect.arrayContaining(["weave_work", "weave_ship", "weave_quick", "weave_overview", "weave_close_session", "weave_breadcrumbs", "weave_plan"])
    );
    expect(tools).toHaveLength(7);
  });
});
...
describe("Weave MCP Server --scope=inspect", () => {
  ...
  it("should only expose inspect tools", async () => {
    ...
    expect(toolNames).toEqual(
      expect.arrayContaining(["weave_context", "weave_search", "weave_status", "weave_health", "weave_preflight", "weave_sync", "weave_tree", "weave_learnings", "weave_guide"])
    );
