...
export const SCOPE_TOOLS: Record<Exclude<Scope, "all">, string[]> = {
  ...
  session: ["weave_work", "weave_ship", "weave_quick", "weave_overview", "weave_close_session", "weave_breadcrumbs", "weave_plan"],
  inspect: ["weave_context", "weave_search", "weave_status", "weave_health", "weave_preflight", "weave_sync", "weave_tree", "weave_learnings"],
};
...
function parseScope(): Scope {
  ...
  if (!["graph", "session", "inspect", "all"].includes(value)) {
    console.error(
      `Invalid scope "${value}". Valid: graph, session, inspect, all`
    );
    process.exit(1);
  }
  return value;
...
function findWvPath(): string {
  ...
  throw new Error(
    "wv CLI not found. Install with: cd weave && ./install.sh"
  );
}
...
const TOOLS: Tool[] = [
  {
    name: "weave_search",
    description:
      "Full-text search across Weave nodes. Returns matching nodes ranked by relevance.",
    inputSchema: {
      type: "object",
      properties: {
        query: {
  ...
  {
    name: "weave_add",
    description: "Create a new Weave node. Returns the generated node ID. IMPORTANT: Always set gh=true to create a linked GitHub issue, and provide an alias for readability.",
    inputSchema: {
      type: "object",
      properties: {
        text: {
        ...
        gh: {
          type: "boolean",
          description: "Create a linked GitHub issue. Should ALWAYS be true unless explicitly told otherwise -- orphan nodes without GH issues lose traceability.",
        },
        alias: {
          type: "string",
          description: "Human-readable alias (e.g., 'fix-login-bug'). Should ALWAYS be set -- makes the graph readable and commands easier.",
        },
      },
      required: ["text"],
    },
  ...
  {
    name: "weave_done",
    description: "Mark a Weave node as complete. Always include a learning for non-trivial work -- captures decisions, patterns, and pitfalls for future sessions.",
    inputSchema: {
      type: "object",
      properties: {
        id: {
        ...
        learning: {
          type: "string",
          description: "Learning to capture (decision/pattern/pitfall). IMPORTANT: Always provide for non-trivial work.",
        },
        no_warn: {
          type: "boolean",
  ...
  },
  {
    ...
    description: "Close multiple nodes at once. Useful for completing a group of related tasks from a sprint. Applies the same learning to all nodes.",
    inputSchema: {
      type: "object",
      properties: {
        ids: {
  ...
  },
  {
    name: "weave_context",
    description:
      "Get full Context Pack for a node: node details, blockers, ancestors, related nodes, and learnings.",
    inputSchema: {
      type: "object",
      properties: {
        id: {
  ...
  },
  {
    name: "weave_status",
    description:
      "Get compact status summary: active work, ready count, blocked count.",
    inputSchema: {
      type: "object",
      properties: {},
      required: [],
  ...
  },
  {
    name: "weave_health",
    description:
      "Run health check on Weave graph. Returns score and any issues found.",
    inputSchema: {
      type: "object",
      properties: {
        verbose: {
  ...
  },
  {
    name: "weave_work",
    description:
      "Claim a node to work on. Sets WV_ACTIVE context for subagent inheritance.",
    inputSchema: {
      type: "object",
      properties: {
        id: {
  ...
  },
  {
    name: "weave_ship",
    description:
      "Complete current work: mark node done with learning, then sync to git layer. Always include a learning for non-trivial work.",
    inputSchema: {
      type: "object",
      properties: {
        id: {
  ...
  },
  {
    name: "weave_sync",
    description:
      "Persist graph to disk and optionally sync GitHub issues. Call periodically and before session end.",
    inputSchema: {
      type: "object",
      properties: {
        gh: {
  ...
  },
  {
    ...
    inputSchema: {
      ...
      properties: {
        ...
        metadata: {
          type: "object",
          description: "JSON metadata to merge into existing keys (e.g., {commit: 'abc123'}). Existing keys are preserved.",
        },
        alias: {
          type: "string",
  ...
  },
  {
    ...
    description:
      "Save, show, or clear session breadcrumbs. Use to leave context notes for future sessions or agents.",
    inputSchema: {
      type: "object",
      properties: {
        action: {
...
function handleTool(
  ...
  switch (name) {
    ...
    case "weave_overview": {
      const parts: string[] = [];
      try { parts.push("=== Status ===\n" + wv(["status"])); } catch { /* skip */ }
      try { parts.push("\n=== Digest ===\n" + wv(["digest"])); } catch { /* skip */ }
      try { parts.push("\n=== Breadcrumbs ===\n" + wv(["breadcrumbs", "show"])); } catch { /* skip */ }
      try { parts.push("\n=== Ready Work ===\n" + wv(["ready"])); } catch { /* skip */ }
      ...
      try {
        ...
        try { accessSync(devPath, constants.X_OK); scriptPath = devPath; } catch { /* not dev */ }
        if (!scriptPath) {
          try { accessSync(installedPath, constants.X_OK); scriptPath = installedPath; } catch { /* not installed */ }
        }
        if (scriptPath) {
      ...
      } catch { /* skip — context-guard.sh execution failed */ }
      result = parts.join("\n");
      break;
    }
...
async function main() {
  ...
  console.error(
    `Weave MCP server started (scope=${ACTIVE_SCOPE}, ${SCOPED_TOOLS.length} tools)`
  );
}
