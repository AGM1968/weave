...
name: plan-agent
description:
  "Automate cold-start graph enrichment: audit repo, generate PLAN.md, import to graph, enrich nodes
  with code references and learnings, sync GitHub, render Mermaid."
---
...
1. **Render Mermaid graph** for each epic:
   ...
   Or use `renderMermaidDiagram` in MCP context with color coding:
   - done = green, active = blue, blocked = red, todo = gray
...
4. **Mermaid generation:** `wv tree --mermaid` generates the graph automatically. Use
   `renderMermaidDiagram` in MCP context for visual rendering.
