...
2. If no children:
   → Suggest: "This epic has no breakdown. Invoke epic-planner?"
   → If yes: runSubagent(epic-planner) with epic context
   → Agent returns suggested feature/task structure
...
#### Mode 1: Node ID Provided (`/weave wv-xxxx`)
...
2. **Create node:**
   ...
   - Captures returned ID (wv-xxxx format)
...
### Learning Curator Auto-Invocation

**Always invoked** in CLOSE phase (not optional) to ensure learnings are captured:
...
5. Learnings stored in node metadata
```

**Why not optional:** Learnings are the primary value of Weave. Skipping them defeats the purpose of
graph-first workflow. Non-trivial work MUST have learnings.
