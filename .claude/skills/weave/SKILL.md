...
If `wv ship` or `wv sync` is interrupted (crash, timeout), `wv recover` resumes from the journal.
Recovery is also triggered automatically on `wv init` and `wv ship`. Use `wv recover --auto` for
non-interactive recovery in hooks/scripts.
