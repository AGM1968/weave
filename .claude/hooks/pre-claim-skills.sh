...
if [[ "$COMMAND" =~ wv[[:space:]]update[[:space:]]wv-[0-9a-f]{4}.*--status=active ]]; then
    # Extract the node ID
    ...
    if [ -x "$WV" ]; then
        ...
        HAS_SHIP_IT=$("$WV" show "$NODE_ID" --json 2>/dev/null | jq -r '.[0]."json(metadata)" | fromjson | has("done_criteria")' 2>/dev/null || echo "false")
        HAS_PRE_MORTEM=$("$WV" show "$NODE_ID" --json 2>/dev/null | jq -r '.[0]."json(metadata)" | fromjson | has("risks")' 2>/dev/null || echo "false")
