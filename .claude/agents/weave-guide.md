...
# Claim a node
wv update wv-XXXX --status=active

# Understand context
wv show wv-XXXX
wv path wv-XXXX  # See dependency chain
```
...
wv done wv-XXXX --learning="pattern: Reusable technique or approach that worked"
```
...
git add .weave/
git commit -m "feat: completed wv-XXXX with learnings"
git push
```
...
wv block wv-XXXX --by=wv-YYYY  # Epic blocked by feature
```
...
wv block wv-YYYY --by=wv-ZZZZ  # Feature blocked by task
```
...
wv show wv-XXXX  # Shows "Blocked by:" section

# See full dependency chain
wv path wv-XXXX  # Shows recursive dependencies

# Chain format
wv path wv-XXXX --format=chain
...
```bash
wv done wv-XXXX --learning="pattern: Delegate exploratory searches to specialized agents to reduce context usage"
```
...
```bash
wv done wv-XXXX  # No learnings captured
```
...
wv done wv-XXXX --learning="pattern: ..."
```
