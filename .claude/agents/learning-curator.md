...
git log --all --grep="wv-XXXXXX" --oneline

# View commit details
git show <commit-hash>

# Check for related PRs or issues
gh pr list --search "wv-XXXXXX"
```
...
EXISTING=$(wv show wv-XXXXXX --json | jq '.[0].metadata')
wv update wv-XXXXXX --metadata="$(echo $EXISTING | jq '. + {
  "decision": "...",
  "pattern": "...",
  "pitfall": "..."
}')"
...
for node in $nodes; do
  ...
  if [ "$response" = "y" ]; then
    ...
    wv update $node --metadata="$(wv show $node --json | jq --arg d "$decision" --arg p "$pattern" --arg f "$pitfall" \
      '.[0].metadata | fromjson | . + {decision: $d, pattern: $p, pitfall: $f}')"
