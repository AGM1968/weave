...
wv add "<description>" --gh  # Create new node + GitHub issue
...
When working on epics or multi-task features, generate Mermaid dependency/progress graphs.
This is what makes Weave a graph system, not a flat checklist.
...
5. **Piping `db_query_json` into `jq`** — Causes SIGPIPE under `set -eo pipefail`; use intermediate variable
