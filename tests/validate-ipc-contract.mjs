...
addFormats(ajv);
for (const name of fs.readdirSync(contracts).filter((name) => name.endsWith(".schema.json")))
  ajv.addSchema(read(path.join(contracts, name)));
...
for (const vector of canonicalJsonV1.invalid_vectors) {
  ...
  if (vector.input_json !== undefined) {
    try {
      value = JSON.parse(vector.input_json);
    } catch {
      rejected = true;
    }
...
const validate = (schema, value, label) => {
  ...
  assert(fn, `missing schema ${schema}`);
  assert(fn(value), `${label}: ${ajv.errorsText(fn.errors)}`);
};
...
const deltaPatchFields = Object.keys(
  deltaOperationV2Schema.properties.payload.properties.mutation.properties.fields.properties
).sort();
assert(isDeepStrictEqual(registryFields, deltaPatchFields), "Delta v2 patch fields must match node field registry");
assert(
  isDeepStrictEqual(nodeFieldRegistry.fields.status.value.enum, deltaOperationV2Schema.$defs.status_value.enum),
  "status registry/schema enum mismatch"
);
assert(
  isDeepStrictEqual(nodeFieldRegistry.fields.risk_level.value.enum, deltaOperationV2Schema.$defs.risk_level_value.enum),
  "risk_level registry/schema enum mismatch"
);
assert(
  isDeepStrictEqual(nodeFieldRegistry.fields.claimed_by.value.type, ["string", "null"]),
  "claimed_by registry type mismatch"
);
assert(nodeFieldRegistry.fields.risk_level.absent === "default:none", "risk_level absent value must normalize to none");
assert(
  deltaOperationV2Schema.$defs.claimed_by_value.oneOf.some((value) => value.type === "string" && value.minLength === 1),
  "claimed_by schema must permit non-empty strings"
);
assert(
  deltaOperationV2Schema.$defs.claimed_by_value.oneOf.some((value) => value.type === "null"),
  "claimed_by schema must permit null"
);
...
const verifyCheckpointBundle = () => {
  ...
  assert(
    isDeepStrictEqual(collectBundleFiles(bundle).sort(), expectedBundleFiles),
    "checkpoint bundle must contain exactly the generated artifacts"
  );
  ...
  assert(
    sha256File(path.join(bundle, generation.state_path)) === manifest.state_sha256,
    "checkpoint state hash mismatch"
  );
  assert(
    sha256File(path.join(bundle, generation.manifest_path)) === generation.manifest_sha256,
  ...
  for (const [key, entry] of Object.entries(manifest.incorporated_legacy_deltas)) {
    assert(
      key === entry.original_path.replace(/^deltas\//, ""),
      `manifest key must match source-relative path for ${key}`
    );
  ...
  assert(
    catalog.entries.length === Object.keys(manifest.incorporated_legacy_deltas).length,
    "legacy catalog has entries outside manifest coverage"
  );
};
...
for (const entry of catalog.errors)
  assert(errorPolicy(entry), `catalog ${entry.code}: ${ajv.errorsText(errorPolicy.errors)}`);
for (const entry of catalog.errors) {
...
assert(
  registry.operations.every((operation) => operation.pure === true),
  "v1 operations must be pure"
);
...
for (const operation of registry.operations) {
  ...
  assert(
    ["none", "snapshot", "exact_revision"].includes(operation.consistency),
    `${operation.name} has invalid consistency`
  );
  assert(["none", "bounded", "required"].includes(operation.bounds), `${operation.name} has invalid bounds`);
  assert(
    (operation.bounds === "none") === (operation.counted_result_field === undefined),
    `${operation.name}: counted result field must match bounds policy`
  );
  assert(
    operation.repository_required || operation.durability === "not_applicable",
    `${operation.name} cannot report repository durability`
  );
}
const validateRequest = (request, label) => {
  ...
  if (operation.consistency === "exact_revision")
    assert(
      request.expected_graph_revision !== null && request.expected_graph_revision !== undefined,
      `${label}: exact_revision requires expected_graph_revision`
    );
  const check = ajv.compile({
    $ref: `https://weave.dev/contracts/operation-payloads-v1.schema.json#/$defs/${operation.request_schema}`,
  });
...
const validateResponse = (request, response, label) => {
  ...
  if (response.result !== undefined) {
    const check = ajv.compile({
      $ref: `https://weave.dev/contracts/operation-payloads-v1.schema.json#/$defs/${operation.result_schema}`,
    });
  ...
  if (success && operation.repository_required)
    assert(
      response.graph_revision === response.durability.graph_revision,
      `${label}: response/durability revision mismatch`
    );
  if (operation.name === "evaluate_hook_policy" && response.result !== undefined) {
    ...
    assert(
      response.result.graph_revision === request.payload.event.graph_revision,
      `${label}: verdict graph revision mismatch`
    );
    assert(
      response.result.policy_revision === request.payload.event.policy_revision,
      `${label}: verdict policy revision mismatch`
    );
    assert(
      request.expected_graph_revision === request.payload.event.graph_revision,
      `${label}: expected/event revision mismatch`
    );
    assert(
      response.graph_revision === request.payload.event.graph_revision,
      `${label}: response/event revision mismatch`
    );
    const supplied = new Set(request.payload.evidence.map((evidence) => evidence.evidence_id));
    assert(
      response.result.evidence_ids.every((id) => supplied.has(id)),
      `${label}: verdict uses unknown evidence`
    );
    assert(
      isDeepStrictEqual(request.repository, request.payload.event.repository),
      `${label}: envelope/event repository mismatch`
    );
    ...
    assert(
      request.payload.event.evidence_ids.length === request.payload.evidence.length &&
        request.payload.event.evidence_ids.every((id) => supplied.has(id)),
      `${label}: event evidence mismatch`
    );
    assert(
      request.payload.evidence.every((evidence) => isDeepStrictEqual(evidence.repository, request.repository)),
      `${label}: evidence repository mismatch`
    );
    assert(
      request.payload.evidence.every(
        (evidence) => evidence.policy_fingerprint === request.payload.event.policy_revision
      ),
      `${label}: evidence policy fingerprint mismatch`
    );
  }
  if (operation.name === "durability_status" && success)
    assert(response.result.graph_revision === response.graph_revision, `${label}: durability result revision mismatch`);
  if (success)
    assert(
      operation.repository_required
        ? response.durability !== null && response.durability !== undefined
        : response.durability == null,
      `${label}: durability mismatch`
    );
  else {
    assert(response.bounds === undefined, `${label}: errors must not carry bounds`);
    if (response.error.code === "repository_mismatch")
      assert(
        response.durability == null && response.graph_revision == null,
        `${label}: repository mismatch must not report durability`
      );
    else
      assert(
        (response.durability == null && response.graph_revision == null) ||
          (response.durability != null && response.graph_revision === response.durability.graph_revision),
        `${label}: error durability/revision mismatch`
      );
  }
  if (success && (operation.bounds === "required" || operation.bounds === "bounded")) {
    ...
    assert(
      response.result[operation.counted_result_field].length === response.bounds.returned,
      `${label}: returned count mismatch`
    );
...
for (const required of [
  ...
  assert(names.has(required), `missing v1 operation ${required}`);
for (const operation of registry.operations)
  for (const code of operation.errors) assert(codes.has(code), `${operation.name} references unknown error ${code}`);
const observations = read(path.join(fixtures, "normalized/cli-observations.json")).observations;
for (const observation of observations)
  assert(names.has(observation.operation), `Bash observation maps unknown operation ${observation.operation}`);
const cli = read(path.join(contracts, "cli-compat-v1.json")).mappings;
validate(
  "https://weave.dev/contracts/cli-compat-v1.schema.json",
  read(path.join(contracts, "cli-compat-v1.json")),
  "CLI compatibility map"
);
for (const mapping of cli)
  assert(
    names.has(mapping.operation) && Array.isArray(mapping.argv_template),
    `invalid CLI compatibility mapping ${mapping.operation}`
  );
assert(new Set(cli.map((mapping) => mapping.operation)).size === cli.length, "CLI mappings must be unique");
assert(
  new Set(observations.map((observation) => observation.operation)).size === observations.length,
  "CLI observations must be unique"
);
const mapped = registry.operations
  .filter((operation) => operation.cli_mapping === "mapped")
  .map((operation) => operation.name);
assert(mapped.length === cli.length, "every mapped operation must have one CLI mapping");
assert(
  new Set(mapped).size === cli.length &&
    mapped.every((operation) => cli.some((mapping) => mapping.operation === operation)),
  "CLI map must equal registry mapped set"
);
assert(
  new Set(cli.map((mapping) => mapping.operation)).size === observations.length &&
    observations.every((observation) => cli.some((mapping) => mapping.operation === observation.operation)),
  "CLI observations must equal CLI map"
);
for (const observation of observations) {
  const mapping = cli.find((candidate) => candidate.operation === observation.operation);
  assert(
    isDeepStrictEqual(mapping.argv_template.slice(1), observation.argv),
    `${observation.operation}: CLI argv mismatch`
  );
}
validate(
  "https://weave.dev/contracts/protocol-v1.schema.json",
  read(path.join(fixtures, "valid/handshake-request.json")),
...
validateResponse(
  read(path.join(fixtures, "valid/ping-request.json")),
  read(path.join(fixtures, "valid/ping-response.json")),
  "ping response"
);
validateResponse(
  read(path.join(fixtures, "valid/health-request.json")),
  read(path.join(fixtures, "valid/health-response.json")),
  "health response"
);
for (const name of ["unknown-operation-request.json", "status-without-repository.json"]) {
  let rejected = false;
  try {
    validateRequest(read(path.join(fixtures, "invalid", name)), name);
  } catch {
    rejected = true;
  }
...
validate(
  "https://weave.dev/contracts/protocol-v1.schema.json",
  read(path.join(fixtures, "valid/unsupported-response.json")),
  "valid unsupported response"
);
assert(
  codes.has(read(path.join(fixtures, "valid/unsupported-response.json")).error.code),
  "wire error must resolve in authoritative catalog"
);
validateResponse(
  handshakeRequest,
  read(path.join(fixtures, "valid/unsupported-response.json"), "unsupported response")
);
...
validateResponse(
  statusRequest,
  read(path.join(fixtures, "valid/repository-mismatch-response.json")),
  "repository mismatch error response"
);
let missingDurability = false;
try {
  validateResponse(
    statusRequest,
    read(path.join(fixtures, "invalid/status-response-without-durability.json")),
    "missing durability"
  );
} catch {
  missingDurability = true;
}
assert(missingDurability, "repository response without durability must be rejected");
let missingBounds = false;
try {
  validateResponse(
    read(path.join(fixtures, "valid/context-request.json")),
    read(path.join(fixtures, "invalid/bounded-response-without-bounds.json")),
    "missing bounds"
  );
} catch {
  missingBounds = true;
...
for (const [name, mutate] of [
  [
    "limit mismatch",
    (value) => {
      value.bounds.limit = 11;
    },
  ],
  [
    "returned cardinality mismatch",
    (value) => {
      value.bounds.returned = 0;
    },
  ],
  [
    "total smaller than returned",
    (value) => {
      value.bounds.total_known = 0;
    },
  ],
  [
    "continuation without truncation",
    (value) => {
      value.bounds.continuation = "next";
    },
  ],
]) {
  ...
  let rejected = false;
  try {
    validateResponse(read(path.join(fixtures, "valid/query-nodes-request.json")), invalid, name);
  } catch {
    rejected = true;
  }
  assert(rejected, `${name} must be rejected`);
...
validateResponse(
  read(path.join(fixtures, "valid/bootstrap-request.json")),
  read(path.join(fixtures, "valid/bootstrap-response.json")),
  "bootstrap response"
);
validateResponse(
  read(path.join(fixtures, "valid/context-request.json")),
  read(path.join(fixtures, "valid/context-response.json")),
  "context response"
);
// wv-22ca26: the fixture above is hand-written and drifted from the live shape once
// done_criteria/risks/risk_level (node) and blocked_reason (envelope) were added to
// cmd_context's actual output (wv-cccf70) -- bumping the schema/fixture together
// wouldn't have caught that drift, so also sample a REAL `wv context --json
// --mode=bootstrap` payload from the live graph and validate it against the same
// contract. Node id and bounds are derived at runtime, not hard-coded, so this
// doesn't pin the test to one node that could later be closed/deleted.
{
  const wvBin = path.join(root, "scripts/wv");
  const liveNodes = JSON.parse(execFileSync(wvBin, ["query", "--limit=1", "--format=json"], { encoding: "utf8" }));
  const liveNodeId = liveNodes[0]?.id;
  assert(liveNodeId, "live IPC contract check needs at least one node in the graph to sample");
  const liveResult = JSON.parse(
    execFileSync(wvBin, ["context", liveNodeId, "--json", "--mode=bootstrap"], { encoding: "utf8" })
  );
  const liveLimit = Math.max(10, Math.min(200, liveResult.blockers.length || 1));
  const liveRequest = structuredClone(read(path.join(fixtures, "valid/context-request.json")));
  liveRequest.payload.node_id = liveNodeId;
  liveRequest.payload.limit = liveLimit;
  const liveEnvelope = structuredClone(read(path.join(fixtures, "valid/context-response.json")));
  liveEnvelope.result = liveResult;
  liveEnvelope.bounds.limit = liveLimit;
  liveEnvelope.bounds.returned = liveResult.blockers.length;
  liveEnvelope.bounds.total_known = liveResult.blockers.length;
  liveEnvelope.bounds.truncated = false;
  liveEnvelope.bounds.continuation = null;
  validateResponse(liveRequest, liveEnvelope, `live context response (${liveNodeId}, bootstrap mode)`);
}
validateResponse(
  read(path.join(fixtures, "valid/preflight-request.json")),
  read(path.join(fixtures, "valid/preflight-response.json")),
  "preflight response"
);
...
validateResponse(
  reorderedHookRequest,
  read(path.join(fixtures, "valid/hook-response.json")),
  "hook reordered repository"
);
for (const name of ["hook-actor-mismatch.json", "hook-session-mismatch.json"]) {
  ...
  let rejected = false;
  try {
    validateResponse(mismatchRequest, read(path.join(fixtures, "valid/hook-response.json")), name);
  } catch {
    rejected = true;
  }
  assert(rejected, `${name} must be rejected`);
...
validateResponse(
  read(path.join(fixtures, "valid/query-active-request.json")),
  read(path.join(fixtures, "valid/query-active-response.json")),
  "active-node response"
);
validateResponse(
  read(path.join(fixtures, "valid/query-nodes-request.json")),
  read(path.join(fixtures, "valid/query-nodes-response.json")),
  "node-query response"
);
validateResponse(
  read(path.join(fixtures, "valid/durability-request.json")),
  read(path.join(fixtures, "valid/durability-response.json")),
  "durability response"
);
...
try {
  validateResponse(
    hookRequest,
    read(path.join(fixtures, "invalid/hook-response-unknown-evidence.json")),
    "hook unknown evidence"
  );
} catch {
  unknownHookEvidence = true;
}
...
assert(
  !wireError(read(path.join(fixtures, "invalid/wire-error-with-policy-fields.json"))),
  "wire errors must not duplicate catalog policy"
);
const bad = ajv.getSchema("https://weave.dev/contracts/protocol-v1.schema.json");
assert(
  !bad(read(path.join(fixtures, "invalid/response-with-result-and-error.json"))),
  "invalid response must be rejected"
);
validate(
  "https://weave.dev/contracts/policy-v1.schema.json#/$defs/verdict",
  read(path.join(fixtures, "verdicts/deny-missing-evidence.json")),
  "verdict fixture"
);
validate(
  "https://weave.dev/contracts/policy-v1.schema.json#/$defs/verdict",
  read(path.join(fixtures, "verdicts/allow-fresh-evidence.json")),
  "allow verdict fixture"
);
validate(
  "https://weave.dev/contracts/policy-v1.schema.json#/$defs/verdict",
  read(path.join(fixtures, "verdicts/advisory-stale-evidence.json")),
  "advisory verdict fixture"
);
const verdict = ajv.getSchema("https://weave.dev/contracts/policy-v1.schema.json#/$defs/verdict");
assert(
  !verdict(read(path.join(fixtures, "invalid/verdict-unknown-reason-code.json"))),
  "unknown policy reason code must be rejected"
);
assert(
  !wireError(read(path.join(fixtures, "invalid/stale-revision-detail-opaque.json"))),
  "error detail must satisfy its code-specific schema"
);
const evidenceSchema = ajv.getSchema("https://weave.dev/contracts/evidence-v1.schema.json");
const unavailableEvidence = read(path.join(fixtures, "valid/evidence-unavailable.json"));
validate("https://weave.dev/contracts/evidence-v1.schema.json", unavailableEvidence, "unavailable evidence fixture");
const pathOnlyEvidence = structuredClone(unavailableEvidence);
pathOnlyEvidence.provenance.references = [{ path_hint: "docs/evidence.md" }];
assert(!evidenceSchema(pathOnlyEvidence), "durable evidence must reject a path-only reference");
const undigestedPathEvidence = structuredClone(unavailableEvidence);
undigestedPathEvidence.provenance.references = [
  {
    node_id: "wv-123abc",
    commit_sha: "0123456789abcdef0123456789abcdef01234567",
    path_hint: "docs/evidence.md",
  },
];
assert(!evidenceSchema(undigestedPathEvidence), "byte-sensitive durable evidence must require a digest");
validate(
  "https://weave.dev/contracts/durability-v1.schema.json#/$defs/durability_status",
  read(path.join(fixtures, "durability/serialized-local-only.json")),
  "durability fixture"
);
const durability = ajv.getSchema("https://weave.dev/contracts/durability-v1.schema.json#/$defs/durability_status");
assert(
  !durability(read(path.join(fixtures, "invalid/upstream-without-local-commit.json"))),
  "upstream confirmation without a matching local commit must be rejected"
);
...
validate(
  checkpointManifest,
  read(path.join(fixtures, "durability/checkpoint-manifest.json")),
  "checkpoint manifest fixture"
);
validate(
  legacyCatalog,
  read(path.join(fixtures, "durability/legacy-delta-catalog.json")),
  "legacy delta catalog fixture"
);
...
assert(
  !deltaSchema(read(path.join(fixtures, "invalid/delta-v2-bad-field.json"))),
  "Delta v2 must reject invalid typed field values"
);
...
try {
  validateDeltaOperationV2(deltaBadHash, "delta bad payload hash");
} catch {
  deltaHashRejected = true;
}
...
for (const [field, mutate] of [
  [
    "operation_id",
    (value) => {
      value.operation_id = "018f0000-0000-7000-8000-000000000099";
    },
  ],
  [
    "actor_id",
    (value) => {
      value.actor_id = "replica-b";
    },
  ],
  [
    "actor_sequence",
    (value) => {
      value.actor_sequence = 43;
    },
  ],
]) {
  ...
  let rejected = false;
  try {
    validateDeltaOperationV2(tampered, `delta tampered ${field}`);
  } catch {
    rejected = true;
  }
...
validate(
  "https://weave.dev/contracts/checkpoint-generation-v1.schema.json",
  read(path.join(fixtures, "durability/checkpoint-generation.json")),
  "checkpoint generation fixture"
);
validate(
  "https://weave.dev/contracts/checkpoint-current-v1.schema.json",
  read(path.join(fixtures, "durability/checkpoint-current.json")),
  "checkpoint current fixture"
);
validate(
  "https://weave.dev/contracts/checkpoint-stage-v1.schema.json",
  read(path.join(fixtures, "durability/checkpoint-stage.json")),
  "checkpoint stage fixture"
);
verifyCheckpointBundle();
const invalidCheckpoint = ajv.getSchema(checkpointManifest);
assert(
  !invalidCheckpoint(read(path.join(fixtures, "invalid/checkpoint-manifest-non-archived.json"))),
  "covered legacy SQL must be archived outside executable deltas"
);
...
checkpointTraversal.incorporated_legacy_deltas["2026-07-16/100-legacy.sql"].archived_path =
  "legacy-deltas/v1/../../state.sql";
