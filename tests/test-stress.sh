...
get_id() {
    echo "$1" | grep -oE 'wv-[a-f0-9]{4}' | tail -1
}
...
test_deep_chain() {
    ...
    local node_count
    node_count=$(echo "$path_result" | grep -cE 'wv-[a-f0-9]{4}' || echo "0")
    TESTS_RUN=$((TESTS_RUN + 1))
...
test_large_metadata() {
    ...
    # Verify storage
    local retrieved_len
    retrieved_len=$("$WV" show "$id" --json 2>/dev/null | jq -r '.[0]."json(metadata)" | fromjson | .learning' | wc -c)
    ...
    local after_len
    after_len=$("$WV" show "$id" --json 2>/dev/null | jq -r '.[0]."json(metadata)" | fromjson | .learning' | wc -c)
    TESTS_RUN=$((TESTS_RUN + 1))
...
test_metadata_injection() {
    ...
    local deep_val
    deep_val=$("$WV" show "$id2" --json 2>/dev/null | jq -r '.[0]."json(metadata)" | fromjson | .a.b.c.d.e.f' 2>/dev/null || echo "")
...
test_work_status_model() {
    ...
    # EXPECT-FAIL: wv work sets 'in-progress', should be 'active'
    local status
    status=$("$WV" show "$id" --json 2>/dev/null | jq -r '.[0].status')
    assert_xfail "active" "$status" "wv work sets status to active"

    # Verify visibility in wv list --status=active
    local list_count
    list_count=$("$WV" list --status=active --json 2>/dev/null | jq length 2>/dev/null || echo "0")
    assert_xfail "1" "$list_count" "Active node visible in wv list --status=active"
...
test_update_status_validation() {
    ...
    # EXPECT-FAIL: wv update --status=banana currently succeeds
    local exit_code=0
    "$WV" update "$id" --status=banana >/dev/null 2>&1 || exit_code=$?
    assert_xfail_exit "nonzero" "$exit_code" "wv update rejects invalid status 'banana'"
...
test_prune_age_zero_guard() {
    ...
    # EXPECT-FAIL: --age=0h currently deletes all done nodes
    local exit_code=0
    "$WV" prune --age=0h >/dev/null 2>&1 || exit_code=$?
    assert_xfail_exit "nonzero" "$exit_code" "wv prune --age=0h is rejected"

    # Check if nodes survived (they won't if the bug is present)
    local count
    count=$("$WV" list --all --json 2>/dev/null | jq length)
    assert_xfail "5" "$count" "Nodes survive age=0h prune"

    # Invalid age format
    local xyz_exit=0
    "$WV" prune --age=xyz >/dev/null 2>&1 || xyz_exit=$?
    assert_xfail_exit "nonzero" "$xyz_exit" "wv prune --age=xyz is rejected"
...
test_search_apostrophe() {
    ...
    # EXPECT-FAIL: wv search with apostrophe crashes with FTS5 syntax error
    local result exit_code=0
    result=$("$WV" search "don't" --json 2>&1) || exit_code=$?
    assert_xfail "0" "$exit_code" "FTS5 search with apostrophe doesn't crash"
...
test_ready_json_empty() {
    ...
    # EXPECT-FAIL: returns empty string, not []
    local result
    result=$("$WV" ready --json 2>/dev/null)
    assert_xfail "[]" "$result" "wv ready --json returns [] when empty"

    # Verify it's parseable as a JSON array
    local jq_result
    jq_result=$(echo "$result" | jq 'type' 2>/dev/null || echo "error")
    assert_xfail '"array"' "$jq_result" "wv ready --json output is a JSON array"
...
test_path_diamond_dedup() {
    ...
    # Default format outputs "wv-xxxx: text" per line
    local total_ids unique_ids
    total_ids=$(echo "$path_result" | grep -oE 'wv-[a-f0-9]{4}' | wc -l)
    unique_ids=$(echo "$path_result" | grep -oE 'wv-[a-f0-9]{4}' | sort -u | wc -l)
    assert_xfail "$total_ids" "$unique_ids" "Diamond path has no duplicate nodes (total=$total_ids, unique=$unique_ids)" || true
...
test_context_pitfall_scoping() {
    ...
    # EXPECT-FAIL: currently includes all pitfalls globally
    assert_xfail "true" "$has_pit1" "Context includes own pitfall"
    assert_xfail "false" "$has_pit2" "Context excludes other epic's pitfall"
...
test_health_invalid_status() {
    ...
    # EXPECT-FAIL: health reports 100/100 with invalid status
    assert_xfail_exit "nonzero" "$([ "$score" = "100/100" ] && echo 0 || echo 1)" "Health penalizes invalid status (score: $score)"
...
test_gh_metadata_roundtrip() {
    ...
    local gh_issue
    gh_issue=$("$WV" show "$id" --json 2>/dev/null | jq -r '.[0]."json(metadata)" | fromjson | .gh_issue' 2>/dev/null || echo "null")
