...
#!/bin/bash
# Initialize Weave in a target repository
#
# Usage:
#   wv-init-repo                 # Default: --agent=claude
...
for arg in "$@"; do
    case "$arg" in
        ...
        --help|-h)
            echo "Usage: wv-init-repo [--agent=claude|copilot|all]"
            echo ""
...
if [ "$AGENT" = "claude" ]; then
    # Claude Code: hooks, skills, CLAUDE.md, AGENTS.md, settings.local.json
    mkdir -p "$REPO_ROOT/.claude/hooks"
    mkdir -p "$REPO_ROOT/.claude/skills/breadcrumbs"
    mkdir -p "$REPO_ROOT/.claude/skills/close-session"
    mkdir -p "$REPO_ROOT/.claude/skills/fix-issue"
    mkdir -p "$REPO_ROOT/.claude/skills/resolve-refs"

    for hook in context-guard.sh session-start-context.sh session-end-sync.sh stop-check.sh post-edit-lint.sh pre-compact-context.sh pre-claim-skills.sh pre-close-verification.sh; do
    ...
    if [ ! -f "$REPO_ROOT/.claude/agents/AGENTS.md" ]; then
...
MCPEOF
        ...
        echo -e "  ${YELLOW}⊘${NC} .vscode/mcp.json (already exists, skipped)"
    fi
...
| `wv add <text>`           | Create node (returns wv-xxxx ID)        | `--gh`, `--alias=`, `--status=`            |
...
COPILOTEOF
        ...
        echo -e "  ${YELLOW}⊘${NC} .github/copilot-instructions.md (already exists, skipped)"
    fi
