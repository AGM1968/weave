...
def render_issue_body(
    ...
    *,
    ...
    if child_ids:
        ...
        # Mermaid graph for parent nodes with children
        if include_mermaid:
            mermaid = render_mermaid_from_tree(node.id) or render_mermaid_graph(
                node, child_ids, nodes_by_id
            )
            if mermaid:
...
def render_mermaid_from_tree(root_id: str) -> str:
    """Render Mermaid by delegating to `wv tree --mermaid --root=<id>`.

    Returns an empty string if command execution fails or output is invalid.
    """
    try:
        result = _run(
            [WV_CMD, "tree", "--mermaid", f"--root={root_id}"],
            check=False,
        )
    ...
    start_idx = next(
        (idx for idx, line in enumerate(lines) if line.strip().startswith("graph TD")),
        None,
    )
