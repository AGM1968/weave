...
auto_sync() {
    ...
    sqlite3 "$WV_DB" ".dump" > "$tmp_sql" 2>/dev/null || { rm -f "$tmp_sql" "$tmp_nodes" "$tmp_edges"; return 0; }
...
cmd_sync() {
    ...
    # Dump to SQL text (git-friendly)
    sqlite3 "$WV_DB" ".dump" > "$tmp_sql"
    ...
    if [ "$gh_sync" = true ] || [ "${WV_GH_SYNC:-0}" = "1" ]; then
        if command -v python3 >/dev/null 2>&1; then
            ...
            sqlite3 "$WV_DB" ".dump" > "$tmp_sql2" 2>/dev/null && mv "$tmp_sql2" "$WEAVE_DIR/state.sql" || rm -f "$tmp_sql2"
...
cmd_load() {
    ...
    if [ -f "$WEAVE_DIR/state.sql" ]; then
        ...
        if sqlite3 "$tmp_db" < "$WEAVE_DIR/state.sql" 2>/dev/null && \
...
EOF
        ...
        echo -e "${GREEN}✓${NC} Loaded from $WEAVE_DIR/state.sql"
    else
        db_init
        echo -e "${YELLOW}No state.sql found, initialized empty database${NC}"
    fi
...
cmd_learnings() {
    ...
    if [ -n "$min_quality" ]; then
        where_parts="$where_parts AND COALESCE(json_extract(metadata, '\$.learning_quality'), 0) >= $min_quality"
    fi
    ...
    echo "$results" | jq -c '.[]' | while read -r row; do
        local id text status metadata decision pattern pitfall learning_note
        id=$(echo "$row" | jq -r '.id')
        text=$(echo "$row" | jq -r '.text')
        status=$(echo "$row" | jq -r '.status')
        metadata=$(echo "$row" | jq -r '.metadata // "{}"')

        # metadata may be a string or object
        if [ "${metadata:0:1}" != "{" ]; then
            metadata=$(echo "$metadata" | jq -r '.' 2>/dev/null || echo "{}")
        fi

        decision=$(echo "$metadata" | jq -r '.decision // empty' 2>/dev/null)
        pattern=$(echo "$metadata" | jq -r '.pattern // empty' 2>/dev/null)
        pitfall=$(echo "$metadata" | jq -r '.pitfall // empty' 2>/dev/null)
        learning_note=$(echo "$metadata" | jq -r '.learning // empty' 2>/dev/null)
        ...
        # Show graph relationships if requested
...
cmd_refs() {
    ...
    # 1. Weave node IDs (wv-xxxx) — confidence 0.9
