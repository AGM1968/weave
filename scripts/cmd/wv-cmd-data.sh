...
auto_sync() {
    ...
    sqlite3 "$WV_DB" ".dump" 2>/dev/null | strip_unistr > "$tmp_sql"
...
cmd_sync() {
    ...
    sqlite3 "$WV_DB" ".dump" | strip_unistr > "$tmp_sql"
    ...
    if [ "$gh_sync" = true ] || [ "${WV_GH_SYNC:-0}" = "1" ]; then
        if command -v python3 >/dev/null 2>&1; then
            ...
            sqlite3 "$WV_DB" ".dump" 2>/dev/null | strip_unistr > "$tmp_sql2" && [ -s "$tmp_sql2" ] && mv "$tmp_sql2" "$WEAVE_DIR/state.sql" || rm -f "$tmp_sql2"
