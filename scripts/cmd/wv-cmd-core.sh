...
cmd_init() {
    ...
    if [ "$force" = true ]; then
        ...
        db_init
        echo -e "${GREEN}✓ Reinitialized Weave at $WEAVE_DIR (clean slate)${NC}"
...
cmd_add() {
    ...
    if [ -z "$text" ]; then
        ...
        echo "Usage: wv add \"task description\" [--status=todo|active|done|blocked] [--gh] [--force]" >&2
        return 1
    fi
...
*Synced from Weave*"
            ...
            if [[ "$gh_url" == http* ]]; then
                ...
                echo -e "${GREEN}✓${NC} GitHub issue #$gh_num created"
            else
...
USAGE
    ...
    if [ -t 0 ]; then
        ...
        echo "Usage: echo '[{\"id\":\"wv-xxxx\",\"alias\":\"test\"}]' | wv bulk-update" >&2
        return 1
    fi
    input=$(cat)
...
cmd_list() {
    ...
    if [ -n "$status_filter" ]; then
        case "$status_filter" in
            todo|active|blocked|done|pending) ;; # valid
            *)
                echo -e "${RED}Error: invalid status '$status_filter'${NC}" >&2
                echo "Valid statuses: todo, active, blocked, done, pending" >&2
                return 1
                ;;
        esac
    fi
...
cmd_ship() {
    ...
    if [ -z "$id" ]; then
        echo -e "${RED}Error: node ID required${NC}" >&2
        echo "Usage: wv ship <id> [--learning=\"...\"]" >&2
        return 1
    fi
