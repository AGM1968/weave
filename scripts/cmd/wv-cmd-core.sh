...
cmd_done() {
    ...
    if [ -n "$learning" ]; then
        ...
        # FTS5 dedup check: warn if a similar learning already exists
        local search_terms
        ...
        if [ -n "$search_terms" ]; then
            ...
            " 2>/dev/null || true)
            if [ -n "$fts_match" ]; then
                echo -e "${YELLOW}⚠ Learning may overlap with $fts_match — check wv show $fts_match${NC}" >&2
            fi
        fi
...
cmd_show() {
    ...
    local query="SELECT id, text, status, json(metadata), created_at, updated_at, alias FROM nodes WHERE id='$id';"
...
cmd_ship() {
    ...
    # Auto-detect GH sync need: check if node or parent epic has gh_issue
    ...
    if [ "$gh_flag" = true ]; then
        ...
        meta_raw=$(db_query_json "SELECT metadata FROM nodes WHERE id='$id';" 2>/dev/null || echo "[]")
        local gh_num
        gh_num=$(echo "$meta_raw" | jq -r '.[0].metadata // "{}" | fromjson | .gh_issue // empty' 2>/dev/null || true)
        if [ -n "$gh_num" ] && [ "$gh_num" != "null" ]; then
            needs_gh=true
        else
            # Check parent epic
            local parent_id
            parent_id=$(db_query "SELECT target FROM edges WHERE source='$id' AND type='implements' LIMIT 1;" 2>/dev/null || true)
            if [ -n "$parent_id" ]; then
                local parent_meta
                parent_meta=$(db_query_json "SELECT metadata FROM nodes WHERE id='$parent_id';" 2>/dev/null || echo "[]")
                local parent_gh
                parent_gh=$(echo "$parent_meta" | jq -r '.[0].metadata // "{}" | fromjson | .gh_issue // empty' 2>/dev/null || true)
                if [ -n "$parent_gh" ] && [ "$parent_gh" != "null" ]; then
                    needs_gh=true
                fi
            fi
        fi
