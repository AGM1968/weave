...
cmd_digest() {
    ...
    local total active ready blocked done_c pending
    ...
    if [ "$format" = "json" ]; then
        jq -n \
            ...
            '{nodes: $total, active: $active, ready: $ready, blocked: $blocked,
              done: $done, pending: $pending, alerts: $alerts,
    ...
    # One-liner output
    local summary="📊 ${total} nodes: ${active} active, ${ready} ready, ${blocked} blocked, ${done_c} done"
    if [ -n "$alerts" ]; then
...
cmd_health() {
    ...
    echo "  Active: $active | Ready: $ready | Blocked: $blocked | Done: $done_count | Pending: $pending"
    echo ""
