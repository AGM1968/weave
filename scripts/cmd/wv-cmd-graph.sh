...
cmd_link() {
    ...
    echo -e "${GREEN}✓${NC} Linked: $from --[$edge_type]--> $to (weight: $weight)"
}
...
cmd_context() {
    ...
    if [ -z "$id" ]; then
        if [ -n "${WV_ACTIVE:-}" ]; then
            ...
            echo "  export WV_ACTIVE=wv-xxxx  # or use: wv work <id>" >&2
            return 1
        fi
    fi
    ...
    jq -n \
        ...
        '{
            ...
            ancestors: ($ancestors | map({
                ...
                status,
                learnings: (
                    (."json(metadata)" // .metadata | if type == "string" then fromjson else . end) as $meta |
                    {
            ...
            pitfalls: ($pitfalls[0:3] | map({
                id,
                text,
                pitfall: ((."json(metadata)" // .metadata | if type == "string" then fromjson else . end).pitfall)
            })),
...
cmd_tree() {
    ...
    sqlite3 -batch -cmd ".timeout 5000" -separator $'\x1f' "$WV_DB" "$query" | while IFS=$'\x1f' read -r id text status node_type depth root_id; do
        ...
        echo -e "${indent}${indicator} ${type_label}: ${text} ${CYAN}(${id})${NC} [${status}]"
    done
}
