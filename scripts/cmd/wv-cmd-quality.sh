...
#
# Commands: quality scan, quality hotspots, quality diff, quality promote, quality reset
...
Subcommands:
  scan [path]    Scan codebase for quality metrics
  reset          Delete quality.db for recovery
  hotspots       Ranked hotspot report (Sprint 4)
  diff           Delta report vs previous scan (Sprint 4)
  promote        Create summary nodes from findings (Sprint 4)

Options:
  --json         JSON output (scan, hotspots, diff)
...
cmd_quality_scan() {
    ...
    while [ $# -gt 0 ]; do
        case "$1" in
            --json)  json_flag="--json" ;;
            --help|-h)
                echo "Usage: wv quality scan [path] [--json]" >&2
                return 0
                ;;
            *)
                if [ -z "$scan_path" ]; then
...
cmd_quality_promote() {
    ...
    while [ $# -gt 0 ]; do
        case "$1" in
            ...
            --help|-h)
                echo "Usage: wv quality promote --parent=<node-id> [--top=N] [--json] [--dry-run]" >&2
                return 0
                ;;
            *)
                echo -e "${RED}Unexpected argument: $1${NC}" >&2
