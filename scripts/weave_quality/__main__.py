...
def cmd_scan(args: argparse.Namespace) -> int:
    ...
    # Discover files
    all_files = _discover_files(repo, exclude_globs=getattr(args, 'exclude', []))
    ...
    for rel_path in all_files:
        ...
        try:
            ...
            mtime = 0
        blob = git_blob_sha(repo, rel_path)
    ...
    for rel_path in files_to_scan:
        abs_path = os.path.join(repo, rel_path)
        if rel_path.endswith(".py"):
            entry, ck = analyze_python_file(abs_path, scan_id)
            entry = FileEntry(
                path=rel_path,
                scan_id=scan_id,
    ...
    for rel_path in files_to_scan:
        fs = build_file_state(repo, rel_path)
    ...
    if json_output:
        ...
        if not ranked:
            ...
            for i, gs in enumerate(ranked, 1):
                ...
                print(
                    f"  {i}. {gs.path:<50s} "
                    f"hotspot={gs.hotspot:.2f}  CC={cc:.0f}  "
...
def cmd_promote(args: argparse.Namespace) -> int:
    ...
    # Check for existing promoted nodes (idempotency)
    rc, existing_json = _wv_cmd("list", "--json", "--all")
    existing_findings: set[str] = set()
    ...
    if json_output:
        ...
        if not promoted and not dry_run:
...
def main() -> int:
    ...
    scan_parser.add_argument("--exclude", action="append", default=[], help="Exclude files matching glob (repeatable)")

    # hotspots
    hotspots_parser = sub.add_parser("hotspots", help="Ranked hotspot report")
    hotspots_parser.add_argument("--top", type=int, default=10, help="Number of results (default: 10)")
    ...
    promote_parser.add_argument("--top", type=int, default=5, help="Number of findings (default: 5)")
    promote_parser.add_argument("--parent", required=True, help="Parent node ID to link via references")
    ...
    elif args.command == "health-info":
        return cmd_health_info(args)
    elif args.command == "context-files":
