...
class FunctionDetail:
    """Per-function metrics collected during single-pass AST walk.

    parent_is_class: True when function is a direct method of a class.
    Used for correct WMC derivation (D1=Option A: only sum class methods).
    """
...
@dataclass
class ASTAnalysis:
    """Combined result container from single-pass AST walk.

    Replaces 7 separate ast.walk calls with one pass that collects:
    - File-level: complexity, nesting, function count/lengths, imports, classes
    - Per-function: CC, EV, dispatch flag, parent-class flag
    - CK-ready: class nodes, import set for downstream CK metric computation
    """
    ...
    def function_count(self) -> int:
        """Number of functions found in the AST."""
    ...
    @property
    def max_essential_complexity(self) -> float:
        """Maximum essential complexity across all functions."""
        evs = [f.essential_complexity for f in self.functions]
        return max(evs) if evs else 1.0

    @property
    def wmc(self) -> float:
        """Weighted Methods per Class: sum CC of class methods only."""
        return sum(f.complexity for f in self.functions if f.parent_is_class)
...
class GitStats:
    ...
    def from_dict(cls, d: dict[str, Any]) -> "GitStats":
        ...
        return cls(
            ...
            ownership_fraction=float(d.get("ownership_fraction", 0.0)),
            minor_contributors=int(d.get("minor_contributors", 0)),
        )
