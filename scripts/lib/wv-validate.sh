...
validate_id() {
    ...
    echo -e "${RED}Error: invalid node ID or alias '$id' (expected wv-XXXX or alias)${NC}" >&2
    return 1
}

# Resolve an alias or ID to the canonical wv-xxxx ID.
# Returns the input unchanged if it's already a wv-xxxx ID.
resolve_id() {
    local id="$1"
    # Accept 4-char (legacy) and 6-char (v1.2+) IDs
    if [[ "$id" =~ ^wv-[a-f0-9]{4,6}$ ]]; then
...
VALID_STATUSES="todo active done blocked"
